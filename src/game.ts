...
export class Game {
    ...
    time: { start: number; elapsed: number; level: number } = { start: 0, elapsed: 0, level: 1000 };

    constructor(board: Board, nextCtx: CanvasRenderingContext2D) {
        this.board = board;
    ...
    handleKey(event: KeyboardEvent) {
        ...
        switch (event.key) {
            case KEY.LEFT:
                p.x--;
                if (this.board.valid(p)) this.activePiece.x--;
                break;
            case KEY.RIGHT:
                p.x++;
                if (this.board.valid(p)) this.activePiece.x++;
    ...
    rotate(cw: boolean) {
        ...
        for (const [kx, ky] of kicks) {
            ...
            if (this.board.valid(p)) {
                if (cw) this.activePiece.rotateCW();
                else this.activePiece.rotateCCW();
